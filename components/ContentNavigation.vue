<template>
  <ul class="nav pe-3">
    <li v-for="(item, index) in navigationTree" :key="index" class="py-0.5">
      <div class="">
        <div v-if="item.children" class="block relative p-1 font-bold cursor-pointer group" @click="showChildren = !showChildren">
          {{ item.title }}
          <span class="flex absolute top-2 right-3 justify-center items-center w-5 h-5 text-xl font-thin rounded-sm group-hover:bg-black group-hover:text-white dark:group-hover:bg-white dark:group-hover:text-black">
